...
export class HttpRequest {
  ...
  constructor(host: null | string = "", prefix: null | string = "", errorHandler?: (statusCode: number, error: HttpHandler) => void) {
  ...
  async request(api: string, data: JSONobj, options: Options) {
    ...
    try {
      const res = await fetch(`${this.host}${this.prefix}${api}${queryString}`, defaultOptions);
