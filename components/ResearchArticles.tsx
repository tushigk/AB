...
export default function ResearchArticles({ textContent }: ResearchArticlesProps) {
  return (
    <section className="md:max-w-4/5 max-w-full mx-auto py-12 px-12">
      <h2 className="text-3xl font-heading font-bold text-foreground mb-4">Мэдээ мэдээлэл</h2>
      <div className="grid grid-cols-2 md:grid-cols-2 gap-6">
        {textContent.map((item) => (
          <div key={item.id} className="relative bg-background border border-foreground/20 rounded-lg p-4 hover:shadow-lg hover:shadow-primary/20 transition">
            <div className="absolute top-2 right-2 bg-primary text-white text-xs px-2 py-1 rounded-full">18+</div>
            <img src={item.image} alt={item.title} className="w-full h-72 object-cover rounded-lg mb-4" />
